...
switch (level) {
  ...
  case 9:
    "winner!";
    for(int i = 0; i < sizeof(winnerSong) / sizeof(winnerSong[0]); i++) 
    {
...
if (playerPixelZeroColor = randomPixelZero) {
  for(int i = 0; i < sizeof(plusPoint) / sizeof(plusPoint[0]); i++) 
    {
    CircuitPlayground.playTone(midi[plusPoint[i][0]], plusPoint[i][1]);
    delay(1);
    }
  totalPoints = t++;
  Serial.print("Total Points: "); Serial.println(totalPoints);
} else {
  for(int i = 0; i < sizeof(minusPoint) / sizeof(minusPoint[0]); i++) 
    {
    CircuitPlayground.playTone(midi[minusPoint[i][0]], minusPoint[i][1]);
    delay(1);
    }
  totalPoints = t--;
  Serial.print("Total Points: "); Serial.println(totalPoints);
